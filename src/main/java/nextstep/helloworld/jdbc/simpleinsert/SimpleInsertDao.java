...
public class SimpleInsertDao {
    ...
    public Customer insertWithMap(Customer customer) {
        ...
        Number number = insertActor.executeAndReturnKey(map);
        return new Customer(number.intValue(), customer.getFirstName(), customer.getLastName());
    }
    ...
    public Customer insertWithBeanPropertySqlParameterSource(Customer customer) {
        ...
        Number number = insertActor.executeAndReturnKey(paramSource);
        return new Customer(number.intValue(), customer.getFirstName(), customer.getLastName());
    }
}
